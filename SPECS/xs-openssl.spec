...
# Citrix: '--generate-missing-build-notes' is not supported in XenServer
# CentOS 7 based build.
%if 0%{?rhel} > 7
...
# Citrix: the file depend on non-available package crypto-policies in CentOS 7.
%if 0%{?rhel} > 7
SYS_CIPHERS_FILE=--system-ciphers-file=%{_sysconfdir}/crypto-policies/back-ends/openssl.config
%else
SYS_CIPHERS_FILE=
%endif
...
# Citrix: below files were included in 'xs-openssl-libs' package.
# But they would conflict with files installed by 'openssl-libs' from CentOS,
# as both libs could be installed on XenServer at same time.
